...
module.exports = {
  name: "pesquisar",
  description: `Procure a musica pelo nome`,
  ...
  options: [
    {
      name: "song",
      description: `diga url/nome da música para reproduzir`,
      type: "STRING",
      required: true,
    },
  ],
  ...
  run: async (client, interaction, args, queue) => {
    ...
    let embed = new EmbedBuilder()
      ...
      .setTitle(`\`${query}\` Resultados da Pesquisa`)
    ...
    let menuraw = new ActionRowBuilder().addComponents([
      new SelectMenuBuilder()
        .setCustomId("search")
        .setPlaceholder(`Clique para ver as melhores músicas`)
        .addOptions(
          res.map((song, index) => {
            return {
              label: song.name.substring(0, 50),
              value: song.url,
              description: `Clique para reproduzir música`,
