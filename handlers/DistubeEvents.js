        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setAuthor({
                name: `Playlist adicionado na fila`,
      ...
      queue.textChannel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setTitle(`Musica nao encontrada \`${queue?.songs[0].name}\``),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
      ...
      queue.textChannel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`A fila acabou! Não há mais música para tocar`),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
      ...
      message.channel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`Eu não posso procurar \`${quary}\``),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(
                `${client.config.emoji.ERROR} Nenhum resultado encontrado para \`${quary}\`!`
              ),
          ],
        })
        .then((msg) => {
        ...
        queue.textChannel
          .send({
            embeds: [
              new EmbedBuilder()
                .setColor(client.config.embed.color)
                .setDescription("O canal está vazio. Saindo do canal"),
            ],
          })
          .then((msg) => {
            setTimeout(() => {
          ...
          case "autoplay":
            {
              if (!channel) {
                send(
                  interaction,
                  `** ${client.config.emoji.ERROR} Você precisa estar em um canal de voz**`
                );
              } else if (
                ...
                send(
                  interaction,
                  `** ${client.config.emoji.ERROR} Você precisa se juntar ao __Meu__ canal de voz **`
                );
              } else if (!queue) {
                send(
                  interaction,
                  `** ${client.config.emoji.ERROR} Eu não estou jogando agora**`
                );
              } else if (!queue.autoplay) {
                queue.toggleAutoplay();
                send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} Autoplay Ligado !! **`
                );
              } else {
                queue.toggleAutoplay();
                send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} Autoplay Está Desligado !!**.`
                );
              }
            }
            break;
          case "skip":
            {
              if (!channel) {
                send(
                  interaction,
                  `** ${client.config.emoji.ERROR} Você precisa está num canal de voz**`
                );
              } else if (
                interaction.guild.members.me.voice.channel &&
                !interaction.guild.members.me.voice.channel.equals(channel)
                ...
                send(
                  interaction,
                  `**${client.config.emoji.SUCCESS} Pule para a próxima canção**.`
                );
              }
            }
            break;
          case "stop":
            {
              if (!channel) {
                send(
                  interaction,
                  `** ${client.config.emoji.ERROR} Você precisa participar do canal de voz**`
                );
              } else if (
                interaction.guild.members.me.voice.channel &&
                !interaction.guild.members.me.voice.channel.equals(channel)
                ...
                send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} Musica Parada e Canal Esquerdo !!**.`
                );
              }
            }
            break;
          case "pauseresume":
            {
              if (!channel) {
                ...
                send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} Fila retomada!! **`
                );
              } else if (!queue.paused) {
                await queue.pause();
                send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} Fila Pausada !! **`
                );
              }
            }
            break;
      ...
      if (message.channelId === musicchannel.id) {
        ...
        if (message.author.bot) {
          ...
          if (
            ...
            return client.embed(
              message,
              `** ${client.config.emoji.ERROR} Você preicsa estar em um canal de voz**`
            );
          } else if (
